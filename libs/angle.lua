--IDIOT. MAKE SURE Y VALUES ARE WORKING.
angle = class:new()

function angle:init(args)
	local x = args[1]
	local y = -args[2]
	if x == 0 then x = 0.001 end --this is so dumb, but it works
	if y == 0 then y = 0.001 end
	theta = math.atan(y/x)
	if x < 0 then
		theta = -theta + math.pi
	end
...
function angle:setTheta(theta)
	...
	local xSign = 1
	local ySign = 1
	if theta > math.pi/2 and theta < 3*math.pi/2 then xSign = -1 end
	if theta > math.pi then ySign = -1 end
